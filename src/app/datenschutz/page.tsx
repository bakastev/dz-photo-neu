...
export const metadata: Metadata = {
  title: 'Datenschutzerklärung | DZ-Photo',
  description: 'Datenschutzerklärung für Daniel Zangerle - DZ-Photo',
  robots: {
    index: true,
    follow: true,
  },
...
async function getDatenschutzPage() {
  try {
    const { data, error } = await supabase
      .from('pages')
      .select('*')
      .eq('slug', 'datenschutzerklaerung')
      .eq('published', true)
      .single();

    if (error || !data) {
      console.error('Error fetching datenschutz page:', error);
      return null;
    }
